<?php
	...
	if (isset($_POST['insert-bus'])) {
		...
		if ($admin=="" || $category=="" || $source=="" || $destination=="" || $title=="" || $intermediate=="" || $date=="" || $via_time=="" || $bus_detail=="" || $max_seats=="" || $seat_cost=="") {
			echo "**All Fields Mandatory";
		}
		else {
			$query = "INSERT INTO posts(post_category_id, post_title, post_author, post_date, post_image, post_content, post_source, post_destination, post_via, post_via_time, max_seats, seat_cost, available_seats)
			VALUES({$category}, '{$title}', '{$admin}', '{$date}', '{$image}', '{$bus_detail}', '{$source}', '{$destination}', '{$intermediate}', '{$via_time}', $max_seats, $seat_cost, $max_seats)";
...
<form id="add_bus_form" action="" method="post" enctype="multipart/form-data">
	...
	<div class="form-group">
		<label for="seat_cost">Cost per Ticket</label>
		<input type="text" class="form-control" name="seat_cost" placeholder="Cost per 1 Ticket">
	</div>
	...
	<div class="form-group">
		<input type="submit" onclick="addBus()" class="btn btn-primary" name="insert-bus" value="Add Bus">
	</div>
</form>
